...
Each browser is a device: it keeps a random id in localStorage (browsers expose no MAC or stable IP; clearing site data makes it a new device), so every tab and window of that browser shares one profile, while a private window or another browser is its own device. It registers with a guessed name (`Android phone`, `iPad`, `Linux PC`) and kind: **touch screen** when touch is the primary input (phones, tablets — even one requesting the desktop site) or **desktop** (mouse or trackpad, including touchscreen laptops). A new device uses the profile for its kind — Touch screen or Desktop. Rename devices, assign any profile to any device, forget old devices, and copy, rename, or delete profiles under **Devices and profiles** on the settings page; the settings below it edit the selected profile.
...
- `GET /v1/stream` — websocket for continuous dictation. First message (text) `{"type":"start","api_key":…,"options":{pause_ms, silence_timeout_s, commands:{send,stop,clear}, start:[phrases], preview, custom_words, remove_fillers, correction_threshold}}`; then binary PCM16 LE 16 kHz mono frames; `{"type":"stop"}` to finish. Server events: `ready`, `partial {seq,text}`, `final {seq,text}`, `command {name: send|stop|clear|start}`, `state {waiting}`, `heard {text}` (speech ignored while waiting), `ended {reason: stopped|silence|command|limit|error}`, `error {message}`. Close codes 4400 (bad start), 4401 (auth), 4503 (loading). Silero VAD endpoints phrases (open after 64 ms of speech, close after `pause_ms`; noise blips under ~100 ms do not restart the pause), partials re-transcribe the open phrase whenever the model is idle, phrases are force-committed at 15 s, sessions end after 4 h. Each phrase's length and close reason (pause, force, finish) is logged; transcripts are not.
